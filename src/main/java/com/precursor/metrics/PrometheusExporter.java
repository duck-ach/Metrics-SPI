...
public final class PrometheusExporter {
    ...
    private PrometheusExporter() {
        ...
        // 로그인 시도 횟수
        loginAttempts = Counter.build().name("keycloak_login_attempts")
                .help("Total login attempts")
                .labelNames("realm", "provider", "client_id").register();
        // 로그인 성공 횟수
        successfulLogins = Counter.build().name("keycloak_logins")
                .help("Total successful logins")
                .labelNames("realm", "provider", "client_id").register();
        // 로그인 실패 횟수
        failedLogins = Counter.build().name("keycloak_failed_login_attempts")
                .help("Total failed login attempts")
                .labelNames("realm", "provider", "error", "client_id").register();
        // 회원가입 성공
        registrations = Counter.build().name("keycloak_registrations")
        ...
        // HTTP 응답 카운터 (전체)
        responseTotal = Counter.build().name("keycloak_response")
